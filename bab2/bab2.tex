...
\textbf{\textit{Model drift}} adalah fenomena penurunan integritas dan akurasi model prediktif seiring berjalannya waktu, yang umumnya disebabkan oleh perubahan distribusi data produksi. Dalam konteks \textit{credit scoring}, jenis \textit{drift} utama yang dikaji pada penelitian ini adalah \textbf{\textit{Data drift}}, yaitu pergeseran pada distribusi input seperti perubahan demografi nasabah, fluktuasi ekonomi makro, atau perubahan kebijakan penyaluran kredit. Jika fenomena ini dibiarkan tanpa intervensi, integritas keputusan model akan terdegradasi secara signifikan, sehingga berpotensi meningkatkan risiko salah klasifikasi---seperti menyetujui nasabah berisiko tinggi atau menolak nasabah yang layak. Oleh karena itu, pendekatan MLOps sangat diperlukan untuk mencegah kerugian finansial akibat model yang usang (\textit{stale model}) melalui mekanisme \textit{monitoring}, \textit{alerting}, dan \textbf{\textit{retraining} model secara otomatis} ketika integritas model terindikasi menurun. \cite{nikolaidis2022credit}
