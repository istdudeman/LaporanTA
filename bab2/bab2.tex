...
Penggunaan WoE dan IV memiliki relevansi langsung dengan implementasi MLOps, karena fitur yang lebih stabil mempermudah konsistensi data selama \textit{Continuous Training}. Dalam konteks ini, stabilitas fitur WoE memungkinkan proses \textit{retraining} otomatis yang dieksekusi secara periodik maupun berbasis akumulasi data baru (\textit{event-based}) berjalan dengan andal tanpa anomali yang ekstrem pada pipeline MLOps.\cite{Chang2024}
...
\subsection{Credit Scoring dan Stale Model}
\textbf{\textit{Stale model}} adalah fenomena penurunan integritas dan akurasi model prediktif seiring berjalannya waktu karena model sudah tidak relevan lagi dengan kondisi data riil atau data produksi yang terus berkembang. Dalam konteks \textit{credit scoring}, model yang dilatih pada masa lalu perlahan akan tertinggal dari dinamika demografi nasabah, fluktuasi ekonomi makro, atau tren risiko kredit terbaru. Jika fenomena ini dibiarkan tanpa intervensi, integritas keputusan model akan terdegradasi secara signifikan, sehingga berpotensi meningkatkan risiko salah klasifikasi---seperti menyetujui nasabah berisiko tinggi atau menolak nasabah yang layak. Oleh karena itu, pendekatan MLOps sangat diperlukan untuk mencegah kerugian finansial akibat \textit{stale model} melalui mekanisme \textbf{\textit{Continuous Training} (CT)} dan \textbf{\textit{retraining} model secara otomatis} yang dijadwalkan secara berkala maupun dipicu oleh akumulasi data produksi baru. \cite{nikolaidis2022credit}
