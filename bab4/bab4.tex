...
Secara umum, pengujian dilakukan melalui enam skenario utama, yaitu pengujian \textit{preprocessing} menggunakan WoE dan IV, penanganan ketidakseimbangan kelas menggunakan SMOTE, pelatihan dan \textit{hyperparameter tuning} model XGBoost, pengujian kualitas data sintetis menggunakan pendekatan TSTR dan TRTR, pengujian layanan inferensi FastAPI beserta aturan bisnis, serta pengujian pipeline \textit{Continuous Training} dan pemicuan \textit{retraining} otomatis.
...
\hline
6 & Continuous Training dan Retraining Otomatis & Menguji mekanisme otomasi pemicuan eksekusi n8n untuk pelatihan ulang (retraining) model. \\
\hline
...
Sebagai gambaran menyeluruh terhadap cakupan fungsi yang diuji, diagram use case sistem ditunjukkan pada Gambar \ref{fig:bab4_use_case_diagram}. Diagram tersebut memperlihatkan interaksi antara pengguna, analis atau pihak penanggung jawab persetujuan, layanan prediksi, orkestrasi n8n, serta komponen pendukung seperti ingestion dokumen, pengiriman konfirmasi, otomasi \textit{Continuous Training}, dan retraining. Dengan demikian, diagram ini menjadi penghubung antara skenario pengujian pada Bab IV dan implementasi pipeline MLOps yang telah dirancang pada Bab III.
...
Dokumentasi parameter pada Tabel \ref{tab:bab4_tuning_summary} ini penting agar proses pelatihan model dapat ditelusuri kembali dan direplikasi.
...
Selain ringkasan metrik evaluasi pada Tabel \ref{tab:bab4_metrik_model}, kurva ROC ditunjukkan pada Gambar \ref{fig:bab4_roc_curve}. Kurva ROC digunakan untuk melihat performa model pada berbagai ambang keputusan, bukan hanya pada satu nilai ambang tertentu.
\begin{figure}[H]
\centering
...
\section{Pengujian Continuous Training dan Retraining Otomatis}
\label{sec:bab4_drift_retraining}
Pengujian ini dilakukan untuk memvalidasi ketahanan dan otomatisasi \textit{pipeline Continuous Training} (CT) yang berjalan di atas n8n. Sistem yang dibangun menggunakan pendekatan \textbf{dual-trigger} untuk memicu \textit{retraining}: \textit{trigger} berbasis jadwal secara periodik (setiap 7 hari) dan \textit{trigger} berbasis \textit{event} saat terjadi akumulasi data nasabah baru pada *database*. Pengujian difokuskan pada simulasi skenario operasional untuk membuktikan bahwa orkestrator n8n mampu secara mandiri menjalankan siklus pembaruan model tanpa memerlukan intervensi manual yang repetitif, serta memiliki \textit{Quality Gate} untuk memastikan hanya model dengan performa (AUC) lebih baik yang diloloskan ke \textit{production}.

\subsection{Pengujian Mekanisme Pemicuan (Trigger)}

Pada skenario pertama, pengujian dilakukan dengan mengaktifkan \textit{Schedule Trigger} pada antarmuka n8n dan menyimulasikan tibanya waktu eksekusi. Hasil log pada n8n menunjukkan bahwa \textit{node execute command} berhasil memanggil skrip \textit{trainingCredit.py}, yang secara sukses menarik data terbaru dari SQLite, melakukan pra-pemrosesan ulang, melatih model XGBoost, dan mencatatkan artefak baru ke dalam \textit{MLflow Tracking Server}. 

Pada skenario kedua, pengujian difokuskan pada \textit{Event-Based Trigger}. Pengujian mensimulasikan injeksi sejumlah data nasabah baru melampaui \textit{ingestion threshold} melalui API. Sistem mencatat lonjakan data, kemudian API mengirimkan instruksi \textit{webhook} secara internal kepada n8n untuk segera menjalankan \textit{workflow retraining}. Respons dari \textit{webhook} menunjukkan status keberhasilan eksekusi \textit{pipeline} dan pembaruan arsitektur model sejalan dengan akumulasi informasi baru, mencerminkan mekanisme \textit{closed-loop} yang responsif.

\subsection{Pengujian Quality Gate dan Model Reloading}

Untuk memvalidasi integritas pengiriman model ke layanan inferensi, skrip \textit{trainingCredit.py} mengimplementasikan tahap komparasi. Dalam pengujian ini, model baru menghasilkan skor AUC sebesar 0.835, yang melampaui ambang minimal 0.75 dan lebih baik dibanding versi produksi sebelumnya. \textit{MLflow Model Registry} mencatat model baru tersebut dengan tahapan "Production", sedangkan model lama dipindahkan ke status "Archived". 

Secara bersamaan, skrip mengirimkan \textit{HTTP POST} ke titik akhir \textit{reload} pada FastAPI (\textit{/reload-model}). Log terminal FastAPI menunjukkan sistem langsung memuat ulang objek model (\textit{hot-reload}) tanpa mematikan layanan API secara keseluruhan, sehingga tidak ada \textit{downtime} yang terjadi selama proses \textit{Continuous Training}.

Implementasi orkestrasi \textit{Continuous Training} otomatis pada n8n ditunjukkan pada Gambar \ref{fig:bab4_n8n_workflow}. Workflow tersebut memuat beberapa komponen utama, yaitu \textit{Schedule Trigger} untuk eksekusi berkala, \textit{Webhook} untuk menerima pemicu akumulasi data, node eksekusi perintah untuk menjalankan proses pelatihan ulang, serta \textit{HTTP Request} untuk menghubungkan proses otomasi dengan layanan inferensi. Dengan struktur tersebut, proses CT dapat dijalankan secara otomatis dari hulu ke hilir.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{bab4/Screenshot 2026-05-25 171559.png}
\caption{Workflow n8n untuk orkestrasi Continuous Training dan retraining otomatis}
...
Bab ini telah menguraikan hasil pengujian sistem mulai dari preprocessing, penanganan imbalance, pelatihan model, evaluasi performa, interpretabilitas, quality gate data sintetis, API serving, database nasabah, ingestion PDF, konfirmasi email HITL, aturan bisnis, hingga otomasi Continuous Training. Hasil pengujian menunjukkan bahwa model XGBoost memperoleh AUC 0,8262 dengan keseimbangan presisi dan recall yang baik. Transformasi WoE dan seleksi IV membantu memperkuat dasar pemilihan fitur, sedangkan SHAP memberikan transparansi terhadap faktor yang memengaruhi prediksi.

Selain performa model, hasil pengujian juga menunjukkan bahwa data sintetis lolos quality gate utilitas dan privasi, layanan FastAPI mampu mengembalikan prediksi beserta penjelasan, database mendukung CRUD dan audit email, workflow n8n mendukung ingestion PDF, aturan bisnis dapat mencegah keputusan yang melanggar kebijakan, dan pipeline Continuous Training mampu memicu pembaruan model secara otomatis. Dengan demikian, sistem yang dikembangkan tidak hanya memenuhi kebutuhan pemodelan prediktif, tetapi juga menunjukkan kesiapan sebagai pipeline MLOps yang operasional, tangguh dari keusangan data, dapat diaudit, dan tetap membutuhkan tata kelola manusia pada kasus yang berisiko.
