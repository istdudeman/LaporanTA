...
Selain mengevaluasi metrik statistik, rancangan monitoring juga memerlukan pencatatan inferensi agar sistem memiliki data produksi yang dapat dibandingkan dengan data referensi. Setiap permintaan ke \textit{endpoint} \textit{/predict} dapat dicatat dalam format log terstruktur yang berisi fitur input, probabilitas \textit{default}, \textit{grade} risiko, dan waktu prediksi. Log inferensi tersebut menjadi dasar untuk menghitung pergeseran distribusi secara berkala, baik pada tingkat skor prediksi maupun fitur individual. Dengan demikian, proses deteksi \textit{drift} tidak hanya bergantung pada data simulasi, tetapi juga dapat diperluas menjadi pemantauan produksi yang bersifat \textit{closed-loop}.
