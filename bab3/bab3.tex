...
Arsitektur sistem dirancang menggunakan pendekatan \textit{pipeline machine learning end-to-end} yang bersifat modular \cite{google2024mlops}. Modularitas ini memungkinkan setiap komponen dikembangkan, diuji, dan dipelihara secara terpisah. Pemisahan tersebut mendukung skalabilitas sistem, mempermudah pelacakan eksperimen, serta mengurangi ketergantungan antarproses ketika terjadi pembaruan model atau perubahan data.
...
\textit{Data Engineering} & Transformasi fitur, validasi struktur data, dan penanganan ketidakseimbangan kelas melalui pendekatan statistik dan teknik oversampling \cite{maurino2025data}. & Python, Pandas, Scikit-learn \\ \hline
\textit{Model Development} & Pelatihan algoritma XGBoost, optimasi \textit{hyperparameter}, dan evaluasi performa model klasifikasi \cite{yotsawat2021improved, idhom2025evaluation}. & XGBoost, RandomizedSearchCV \\ \hline
\textit{Quality Gate} & Validasi utilitas data sintetis, evaluasi kesamaan distribusi, dan pengujian privasi untuk mencegah \textit{memorization} \cite{fca2024synthetic}. & Scipy, Scikit-learn, KNN \\ \hline
\textit{MLOps Automation} & Orkestrasi \textit{workflow}, pelacakan eksperimen, registrasi model, \textit{deployment} API, serta \textit{monitoring} dan \textit{retraining} otomatis \cite{n8n2025docs, Kreuzberger2023}. & n8n, MLflow, FastAPI, Docker \\ \hline
\end{tabularx}
\end{table}
...
Untuk memberikan gambaran yang komprehensif mengenai interaksi antar komponen, Gambar~\ref{fig:overall-pipeline} menyajikan diagram \textit{pipeline} keseluruhan sistem. \textit{Pipeline} ini mengintegrasikan seluruh proses mulai dari penerimaan data hingga pengambilan keputusan kredit.

\begin{figure}[H]
\centering
\begin{tikzpicture}[
    node distance=1.5cm and 2.5cm,
    box/.style={rectangle, draw=black, thick, fill=blue!5, align=center, text width=3.8cm, rounded corners, minimum height=1.2cm},
    db/.style={rectangle, draw=black, thick, fill=orange!10, align=center, text width=3.8cm, minimum height=1.2cm},
    arrow/.style={->, thick, >=stealth}
]

% Kolom 1
\node[box, fill=gray!10] (email) {Aplikasi Kredit\\(Email PDF)};
\node[box, below=of email] (n8n_ingest) {n8n Ingestion\\dan Parse Data};
\node[box, below=of n8n_ingest] (training) {Training dan\\Evaluasi Model};
\node[box, below=of training] (mlflow) {MLflow\\Model Registry};

% Kolom 2
\node[db, right=of n8n_ingest] (database) {Database SQLite\\(Data Nasabah)};
\node[box, right=of mlflow] (fastapi) {FastAPI\\Inference Service};
\node[box, below=of fastapi] (hitl) {Human-in-the-Loop\\(Approval Email)};
\node[box, fill=green!10, below=of hitl] (decision) {Keputusan\\Kredit Final};

% Koneksi
\draw[arrow] (email) -- node[right, font=\footnotesize] {Lampiran} (n8n_ingest);
\draw[arrow] (n8n_ingest) -- node[above, font=\footnotesize] {Simpan Data} (database);
\draw[arrow] ([xshift=-1cm]database.south) |- node[near end, above, font=\footnotesize] {Ekstrak Data Latih} (training.east);
\draw[arrow] (training) -- node[right, font=\footnotesize] {Log dan Artefak} (mlflow);
\draw[arrow] (mlflow) -- node[above, font=\footnotesize] {Load Model} (fastapi);
\draw[arrow] (database.south) -- node[right, near start, font=\footnotesize] {Data Inferensi} (fastapi.north);
\draw[arrow] (fastapi) -- node[right, font=\footnotesize] {Kirim Prediksi} (hitl);
\draw[arrow] (hitl) -- node[right, font=\footnotesize] {Review Manual} (decision);

\end{tikzpicture}
\caption{Diagram \textit{pipeline} keseluruhan sistem yang mencakup \textit{ingestion}, \textit{training}, dan \textit{serving}.}
\label{fig:overall-pipeline}
\end{figure}

Secara garis besar, \textit{pipeline} keseluruhan terbagi menjadi tiga alur utama yang berkesinambungan:
\begin{enumerate}
    \item \textbf{Alur Ingesti Data (\textit{Data Ingestion})}: Berfokus pada penerimaan data nasabah baru dari berbagai sumber, termasuk ekstraksi otomatis dari dokumen PDF melalui email. Data tersebut kemudian distandarisasi dan disimpan ke dalam \textit{database} sebagai sumber kebenaran tunggal (\textit{single source of truth}).
    \item \textbf{Alur Pelatihan Model (\textit{Model Training \& Registry})}: Data historis dari \textit{database} digunakan secara berkala untuk melatih ulang algoritma \textit{machine learning}. Seluruh eksperimen, metrik performa, dan versi model dicatat secara terpusat dalam MLflow untuk menjamin kemudahan audit dan pelacakan versi.
    \item \textbf{Alur Inferensi dan Keputusan (\textit{Serving \& Decisioning})}: Model terbaik yang tersimpan di MLflow digunakan oleh layanan API (FastAPI) untuk menghasilkan prediksi risiko secara \textit{real-time}. Keputusan akhir tetap melibatkan manusia (\textit{Human-in-the-Loop}) melalui konfirmasi interaktif yang diorkestrasi oleh n8n.
\end{enumerate}
...
Penelitian ini menggunakan \textit{Synthetic Data Generation} untuk mengatasi keterbatasan akses terhadap data kredit riil yang dilindungi oleh regulasi perlindungan data pribadi \cite{fca2024synthetic}. Data yang dibangkitkan terdiri atas 10.000 sampel yang merefleksikan karakteristik nasabah perbankan ritel, khususnya segmen ASN dan pensiunan \cite{sarjono2025adaptive}. Pemilihan data sintetis dilakukan agar penelitian tetap dapat merepresentasikan pola umum kredit konsumtif tanpa melanggar aspek kerahasiaan dan kepatuhan hukum.
...
Setelah nilai WoE diperoleh, daya prediksi setiap variabel diukur menggunakan metrik \textit{Information Value} (IV) untuk mengurangi kompleksitas model \cite{maurino2025data}. Variabel dengan nilai IV antara 0.1--0.5 dikategorikan sebagai prediktor menengah hingga kuat. Variabel dengan daya prediksi sangat rendah dieliminasi agar model tidak dipengaruhi oleh fitur yang tidak informatif, sedangkan variabel yang terlalu dominan dievaluasi kembali untuk menghindari potensi \textit{data leakage}. Melalui tahapan ini, himpunan fitur akhir diharapkan mampu menjaga keseimbangan antara akurasi, stabilitas, dan interpretabilitas model.
...
Sistem ini mengimplementasikan \textit{quality gate} yang ketat sebelum model dideploy ke produksi \cite{fca2024synthetic}. Validasi ini bertujuan memastikan bahwa data sintetis tidak hanya menyerupai data asli secara statistik, tetapi juga cukup aman untuk digunakan tanpa membocorkan informasi sensitif. Komponen validasi utama terdiri atas:
\begin{itemize}
    \item \textbf{Validasi Utilitas}: menggunakan metode \textit{Train Synthetic Test Real} (TSTR) dengan ambang batas $\Delta AUC \leq 10\%$ \cite{fca2024synthetic}. Pengujian ini menilai apakah model yang dilatih pada data sintetis masih mampu bekerja dengan baik ketika diuji pada data yang merepresentasikan kondisi riil.
    \item \textbf{Uji Distribusi}: menggunakan \textit{Inverted KS Statistic} dengan ambang batas \textit{p-value} 0.05 \cite{fca2024synthetic}. Pengujian ini bertujuan melihat kesamaan distribusi antarvariabel penting antara data sintetis dan data acuan.
    \item \textbf{Verifikasi Privasi}: menggunakan \textit{Distance to Closest Record} (DCR) untuk memastikan tidak terjadi \textit{memorization} terhadap data asli \cite{fca2024synthetic}. Pengujian ini penting agar data sintetis yang digunakan tetap aman dari risiko reproduksi identitas individual.
\end{itemize}
...
\begin{tikzpicture}[node distance=1.0cm and 2.0cm]
  ...
  \node[brstep, align=center, right=1.5cm of email] (reset) {Reset ke\\approval\_status\\= \textbf{Pending}};
  ...
  \draw[brarrow, dashed, bend left=40] (email.north east) to
    node[above, align=center, font=\footnotesize, yshift=0.1cm]{Kirim ulang\\email} (reset.north);
  \draw[brarrow, dashed] (reset.west) -- (email.east);
\end{tikzpicture}
...
\subsubsection{Continuous Training dan Retraining Otomatis}
Setelah model aktif digunakan pada lingkungan produksi, sistem memerlukan mekanisme pembaruan berkala agar tidak menjadi usang (\textit{stale model}). Pembaruan ini sangat krusial karena pola data pelanggan yang masuk perlahan-lahan berevolusi, sehingga dapat menyebabkan penurunan integritas model prediktif jika tidak diimbangi dengan pengetahuan dari data terbaru. Untuk mencegah degradasi performa seiring berjalannya waktu, n8n diatur untuk secara otomatis memicu proses \textit{retraining} menggunakan kumpulan data paling baru dari database. Mekanisme \textit{Continuous Training} (CT) ini memastikan bahwa model tetap mutakhir (\textit{up-to-date}) dan pembaruan dapat dilakukan secara konsisten tanpa membebani administrator dengan proses pelatihan manual yang repetitif.

Otomasi \textit{Continuous Training} pada sistem ini diimplementasikan menggunakan pendekatan pemicuan ganda (\textit{multiple triggers}) dalam \textit{workflow} n8n. Sistem tidak bergantung pada satu mekanisme tunggal, melainkan menggabungkan jalur pemicuan yang fleksibel:

\begin{enumerate}
    \item \textbf{Trigger Berbasis Jadwal (\textit{Schedule-Based Trigger})} --- n8n secara langsung menjalankan skrip pelatihan ulang (\textit{retraining}) secara periodik setiap \textbf{7 hari sekali}. Jadwal ini memastikan bahwa terlepas dari fluktuasi jumlah data masuk, model akan selalu "belajar" dari kumpulan data terbaru setidaknya satu kali dalam seminggu. Pendekatan ini juga menjamin prediktabilitas penggunaan sumber daya server, karena proses \textit{training} yang berat dapat dijadwalkan pada jam non-sibuk (\textit{maintenance window}).

    \item \textbf{Trigger Berbasis Event dan Manual (\textit{Event-Based \& Manual Trigger})} --- Sebagai pelengkap jadwal mingguan, sistem juga merespons akumulasi data baru. Setiap kali data nasabah baru dikirim melalui API, sistem menambah \textit{counter} data. Apabila jumlah data baru telah melampaui ambang batas konfigurasi (\textit{ingestion threshold}), n8n akan memicu \textit{retraining} agar model dapat segera menyerap informasi baru yang signifikan tersebut. Selain itu, administrator tetap memiliki kontrol untuk memicu proses \textit{retraining} secara manual kapan saja dari dasbor n8n jika diperlukan (misalnya setelah perubahan kebijakan kredit berskala besar).
\end{enumerate}

Kombinasi pendekatan \textit{trigger} ini memastikan pipeline MLOps beroperasi dengan tingkat otomatisasi tinggi (\textit{Continuous Training}), menjaga kebaruan (\textit{freshness}) model, dan secara proaktif mencegah penurunan akurasi tanpa memerlukan modul statistik tambahan yang kompleks pada tahap \textit{inference}. Setiap proses pelatihan ulang tersebut dicatat secara otomatis oleh MLflow, yang bertugas mengevaluasi metrik AUC model baru sebelum mengizinkan proses \textit{deployment} ke FastAPI.

\subsection{Interpretasi Model (XAI) dan Kepatuhan Regulasi}
Untuk menjaga transparansi dan mendukung kebutuhan audit, sistem menggunakan metode SHAP (\textit{SHapley Additive exPlanations}) \cite{ye2025unlocking, rafi2024explainable}. SHAP digunakan untuk menjelaskan kontribusi masing-masing fitur terhadap skor risiko yang dihasilkan model. Penjelasan ini penting karena model risiko kredit tidak hanya dituntut akurat, tetapi juga harus dapat memberikan alasan yang dapat dipahami oleh analis, auditor, maupun pihak regulator.

Dalam konteks implementasi, penjelasan SHAP dapat ditampilkan dalam bentuk kontribusi global maupun lokal. Penjelasan global digunakan untuk mengidentifikasi variabel paling berpengaruh terhadap keputusan model secara keseluruhan, sedangkan penjelasan lokal digunakan untuk menjelaskan alasan di balik prediksi pada satu nasabah tertentu. Pendekatan ini relevan untuk memenuhi kebutuhan kepatuhan, termasuk mandat POJK Nomor 29 Tahun 2024 yang menekankan pentingnya transparansi pada keputusan skor kredit \cite{ojk2024pojk29}. Dengan adanya kemampuan interpretasi tersebut, sistem yang dikembangkan menjadi lebih akuntabel dan lebih siap diadopsi dalam lingkungan operasional yang diatur secara ketat.
