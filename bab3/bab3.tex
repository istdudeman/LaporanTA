...
Database terdiri atas dua tabel. Tabel \texttt{nasabah} merupakan tabel utama yang menyimpan 40 lebih atribut setiap nasabah, mulai dari data identitas, profil pekerjaan, kondisi finansial, parameter risiko hasil model. Tabel \texttt{sent\_emails} berfungsi sebagai log audit yang mencatat setiap pengiriman email konfirmasi kepada pengambil keputusan. Relasi antara kedua tabel ini diikat melalui kolom \texttt{customer\_id} sebagai \textit{foreign key}.
...
GET    & \texttt{/api/nasabah/\{id\}/\allowbreak approve}           & \texttt{update\_approval\_status("Accepted")} \\ \hline
GET    & \texttt{/api/nasabah/\{id\}/\allowbreak reject}            & \texttt{update\_approval\_status("Rejected")} \\ \hline
POST   & \texttt{/api/nasabah/\{id\}/\allowbreak send-confirmation} & \texttt{create\_sent\_email()} \\ \hline
...
\begin{enumerate}
    \item Setelah model menghasilkan prediksi, analis kredit memicu \textit{endpoint} \texttt{POST /api/\allowbreak nasabah/\allowbreak\{id\}/\allowbreak send-confirmation} dengan menyertakan alamat email reviewer.
...
\begin{tikzpicture}[node distance=1.0cm and 2.0cm]
  ...
  \node[brok, align=center, right=3cm of decide] (acc)  {\texttt{approval\_status}\\= \textbf{Accepted}};
  \node[brerr, align=center, left=3cm of decide] (rej)  {\texttt{approval\_status}\\= \textbf{Rejected}};
  \node[brstep, align=center, above=1.2cm of email] (reset) {Reset ke\\\texttt{approval\_status}\\= \textbf{Pending}};
