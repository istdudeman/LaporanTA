...
\node[box, below=1.2cm of n8n_ingest] (training) {Training dan\\Evaluasi Model};
...
\draw[arrow] (database) |- node[near end, above, font=\footnotesize] {Ekstrak Data Latih} (training);
...
\draw[arrow] (database) -- node[right, font=\footnotesize] {Data Inferensi} (fastapi);
...
Setelah model aktif digunakan pada lingkungan produksi, sistem melakukan pemantauan terhadap perubahan distribusi fitur. Pemantauan ini sangat krusial karena pergeseran pola data pelanggan (\textit{data drift}) dapat menyebabkan penurunan integritas model prediktif, yang pada akhirnya mengurangi keandalan keputusan kredit. Pemantauan dilakukan dengan membandingkan distribusi data produksi secara periodik terhadap distribusi data referensi yang digunakan saat pelatihan. Jika indikator \textit{drift} melampaui ambang batas yang telah ditentukan, sistem mendeteksi bahwa model mulai kehilangan relevansinya dengan kondisi riil. Untuk mencegah degradasi performa lebih lanjut, n8n akan secara otomatis memicu proses \textit{retraining} menggunakan data terbaru. Mekanisme ini memastikan bahwa integritas model tetap terjaga dan pembaruan dapat dilakukan secara responsif tanpa harus menunggu evaluasi manual atau audit berkala yang berpotensi terlambat dan menimbulkan kerugian.
